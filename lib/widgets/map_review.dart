...
class _MapReviewState extends State<MapReview> {
  ...
  void moveCam(geolocationBloc) async {
    if (_oldRoute != null && _oldRoute.length > 0) {
      print('move on to ${_oldRoute[_oldRoute.length - 1]}');
      final cameraUpdate =
          CameraUpdate.newLatLng(_oldRoute[_oldRoute.length - 1]);
      final GoogleMapController controller = await _controller.future;
      controller
          .animateCamera(cameraUpdate); //moveCamera to jump rather than animate
    }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<GeolocationBloc, GeoState>(builder: (context, state) {
      if (geolocationBloc.state.status == Status.moveCam &&
          _oldRoute != null &&
          _oldRoute.length > 0) this.moveCam(geolocationBloc);

      return state.status == Status.showSaved || state.status == Status.moveCam
          ? GoogleMap(
              ...
              initialCameraPosition: _oldRoute != null && _oldRoute.length > 0
                  ? CameraPosition(
                      target: _oldRoute[_oldRoute.length - 1], zoom: 14)
                  : CameraPosition(
                      target: LatLng(
